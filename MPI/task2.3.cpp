...
int main(int argc, char **argv) {
    ...
    if (current_process == 0) {
        ...
        for (int i = 0; i < size / part_size; i++, buffer_start_index += part_size) {
            ...
            MPI_Send(&matrix_b[buffer_start_index][0], size * part_size, MPI_INT, i + 1, 0, MPI_COMM_WORLD);
        }
        ...
        int recieved_matrix_b[part_size][size];
        int part_matrix_c[part_size][size];

        MPI_Recv(&recieved_matrix_a[0][0], data_count, MPI_INT, 0, 0, MPI_COMM_WORLD, &status);
        MPI_Recv(&recieved_matrix_b[0][0], data_count, MPI_INT, 0, 0, MPI_COMM_WORLD, &status);

        for (int i = 0; i < part_size; i++) {
            for (int j = 0; j < size; j++) {
                part_matrix_c[i][j] = recieved_matrix_a[i][j] * recieved_matrix_b[i][j];
            }
        }

        MPI_Send(&part_matrix_c, data_count, MPI_INT, 0, 0, MPI_COMM_WORLD);
    }
